...
class ViewController: UITableViewController {
    ...
    var evenNumbers = [UInt64?]()
    var oddNumbers = [UInt64?]()
    ...
    private func loadData() {
        DispatchQueue.global(qos: .userInitiated).async {
            GenerateService.shared.generate() { (index, value) in
                if index%2 == 0 {
    ...
    private func setupView() {
        ...
        self.navigationItem.rightBarButtonItem  = UIBarButtonItem(title: "Fibonacci", style: .done, target: self, action: #selector(fibonacci))
        title = "Primary Number"
        loadData()
    }
    @objc private func fibonacci() {
        let vc = FibonacciVc()
